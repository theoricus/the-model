...
Model = (function() {
  ...
  */
  ...
  Model.configure = function(config) {
    var i, key, method, type, url, _i, _len, _ref, _ref1, _results;
    ...
    for (key in _ref1) {
      type = _ref1[key];
      _results.push(this._config.keys[key] = type);
    }
    return _results;
  };
  ...
  Model.prototype.save = function(callback) {
    ...
    switch (this.constructor._last_action) {
      case "create":
        ...
        req = this.constructor.fetch(url, 'POST', this._keys);
        req.done(function(data) {
          _this.update(data);
          return done(data);
        });
      ...
      case "update":
        url = this.constructor._config.urls.update.replace(/(\:\w+)/, this.id);
        req = this.constructor.fetch(url, 'PUT', this._keys);
        req.done(done);
        return req.error(error);
      case "delete":
        url = this.constructor._config.urls["delete"].replace(/(\:\w+)/, this.id);
